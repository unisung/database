...
<%
  ...
  String sql="select * from board where "+qstr
		    +" like "+"'%"+text+"%'";
  //쿼리문 출력
   ...
   Statement stmt=con.createStatement();//sql문을 전송할 쿼리객체 생성
   ResultSet rs= stmt.executeQuery(sql);
    while(rs.next()){//next() 반복자연할
         int bno  =  rs.getInt("bno");//숫자값 getInt(칼럼명)
      String title=  rs.getString("title");//문자열getString(칼럼명)
